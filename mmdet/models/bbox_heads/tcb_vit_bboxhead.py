...
from ..VIT import ViT, Attention, PreNorm

@HEADS.register_module
class tcb_vit_bboxhead(BBoxSemanticHeadDual):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(tcb_vit_bboxhead, self).__init__(*args, **kwargs)
        ...
        for i in range(self.num_path):
            if self.with_semantic:
                ...
                if self.voc is not None:
                    ...
                    kernel_semantic  = nn.Sequential(
                                        Attention(self.voc.shape[1], heads = 8, head_dim = 64, dropout = 0.1),
                                        nn.Linear(self.voc.shape[1], self.vec_list[0].shape[0])            
                                                    )
                else:
    ...
    def init_weights(self):
        super(tcb_vit_bboxhead, self).init_weights()
            ...
            if self.with_semantic:
                ...
                if self.voc is not None:
                    ...
                    if self.semantic_norm:
                        ...
                        semantic_score = self.kernel_semantic_list[i][0](semantic_score.unsqueeze(1))
                        semantic_score = self.kernel_semantic_list[i][1](semantic_score.squeeze(1))
...
@HEADS.register_module
class TCB_VIT_Bboxhead(tcb_vit_bboxhead):

    def __init__(self, num_fcs=2, fc_out_channels=1024, *args, **kwargs):
        assert num_fcs >= 1
        super(TCB_VIT_Bboxhead, self).__init__(
            num_shared_convs=0,
