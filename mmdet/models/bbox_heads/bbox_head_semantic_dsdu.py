...
class BBoxSemanticHeadDSDU(nn.Module):
    ...
    def __init__(self,
                 ...
                 loss_npair_seen=dict(type='NPairLoss',loss_weight=0.2),
                 loss_npair_unseen=dict(type='NPairLoss',loss_weight=0.2),
                 loss_unseen=dict(type='SoftCrossEntropyLoss',loss_weight=1.0)
                 ):
        super(BBoxSemanticHeadDSDU, self).__init__()
        ...
        #napir loss
        self.loss_npair_seen = build_loss(loss_npair_seen)
        self.loss_npair_unseen = build_loss(loss_npair_unseen)
        self.loss_unseen = build_loss(loss_unseen)
        ...
        if self.with_semantic:
            ...
            #TODO register vec as parameter
            vec = torch.tensor(vec, dtype=torch.float32, requires_grad=True).cuda()
            vec = nn.Parameter(vec)
            self.register_parameter("vec", vec)

            if voc is not None:
                voc = torch.tensor(voc, dtype=torch.float32)
            vec_unseen = torch.tensor(vec_unseen, dtype=torch.float32, requires_grad=True).cuda()
            self.vec = vec.cuda()  # 300*n
            if voc is not None:
                self.voc = voc.cuda()  # 300*66
            else:
               self.voc = None
            #TODO register vec_unseen as parameter   
            vec_unseen = nn.Parameter(vec_unseen)
            self.register_parameter("vec_unseen",vec_unseen)
    ...
    #TODO change hard seen label to soft unseen lable by cosinesimilarity of their vec
    def seen2unseen(self,seen_labels, seen_label_weights):
        d ,n = self.vec_unseen.size()
        l = len(seen_labels)
        unseen_labels = torch.zeros(n,l).cuda()
        unseen_label_weights = seen_label_weights
        for i in range(l):
            seen_label = seen_labels[i]
            if seen_label.data == 0.0:
                continue
            seen_vec = self.vec.data[:,seen_label].reshape(d,1)
            seen_vec = seen_vec.expand(d,n)
            similarity = F.cosine_similarity(seen_vec, self.vec_unseen.data, dim=0, eps=1e-08)
            similarity = F.relu(similarity)
            unseen_labels[:,i] = similarity.t()
        return unseen_labels.t(), unseen_label_weights
    ...
    def loss(self,
        ...
        #TODO softlabel for unseen
        unseen_semantic_score = torch.mm(semantic_score, self.vec.t())
        unseen_semantic_score = torch.mm(unseen_semantic_score, self.vec_unseen)
        unseen_labels = labels.clone()
        unseen_labels_weight = label_weights.clone()
        
        unseen_labels, unseen_labels_weight=self.seen2unseen(unseen_labels, unseen_labels_weight)

        unseen_avg_factor = max(torch.sum(unseen_labels_weight > 0).float().item(), 1.)
        losses['loss_semantic_unseen'] = self.loss_unseen(
                unseen_semantic_score,
                unseen_labels,
                unseen_labels_weight,
                avg_factor=unseen_avg_factor,
                reduction_override=reduction_override)

        #TODO npair loss
        loss_npair_seen = self.loss_npair_seen(self.vec[:,1:])  #self.vec[0] bg vec
        loss_npair_unseen = self.loss_npair_unseen(self.vec_unseen[:,1:])
        losses['loss_npair_seen'] = loss_npair_seen 
        losses['loss_npair_unseen'] =  loss_npair_unseen
