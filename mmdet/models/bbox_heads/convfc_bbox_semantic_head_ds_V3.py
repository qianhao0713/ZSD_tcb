...
class ConvFCSemanticBBoxHeadV3(BBoxSemanticHeadDynamicSeen):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(ConvFCSemanticBBoxHeadV3, self).__init__(*args, **kwargs)
        ...
        vecs = []
        vecs.append(self.vec_fix)
        for i in range(semantic_num):
            if i>0:
                vec_dynamic = torch.tensor(self.vec_fix.clone(), dtype=torch.float32, requires_grad=True).cuda()
                vec_dynamic = nn.Parameter(vec_dynamic)
                self.register_parameter("vec"+str(i), vec_dynamic)
                vecs.append(vec_dynamic)
        self.vec_list = vecs
        ...
        # fusion layer to mix multi-dynamic vecs
        self.fusion_layer = nn.Linear(semantic_num*semantic_dims, semantic_dims,bias=False).cuda()
        self.fusion_layer.weight.requires_grad = True
        nn.init.constant_(self.fusion_layer.weight,0)
        for i in range(semantic_dims):
            nn.init.constant_(self.fusion_layer.weight[i,i],0.5)
            nn.init.constant_(self.fusion_layer.weight[i,i+semantic_dims],0.5)
    ...
    def init_weights(self):
        super(ConvFCSemanticBBoxHeadV3, self).init_weights()
    ...
    def forward(self, x, res_feats=None, context_feats=None, return_feats=False, resturn_center_feats=False, bg_vector=None):
        ...
        if self.with_semantic:
            ...
            if self.voc is not None:
                ...
                semantic_score = torch.mm(semantic_score, semantic_vec)
            else:
                semantic_score = self.kernel_semantic(semantic_vec)
    ...
    def loss(self,
        ...
        #TODO npair loss
        loss_npair = self.loss_npair(self.vec_list[1][:,1:])  #self.vec[0] bg vec
...
#TODO
    ...
    def get_det_bboxes(self,
        ...
        if self.gzsd:
            ...
            scores_0 = torch.mm(seen_scores, self.vec_list[0])
            scores_1 = torch.mm(seen_scores, self.vec_list[1])
            seen_scores = 0.7 * scores_0 + 0.3 * scores_1
        ...
        if self.seen_class:
            ...
            scores_0 = torch.mm(scores, self.vec_list[0])
            scores_1 = torch.mm(scores, self.vec_list[1])
            scores = 0.7 * scores_0 + 0.3 * scores_1
        ...
        if cfg is None:
            ...
            return det_bboxes, det_labels
@HEADS.register_module
class SharedFCSemanticBBoxHeadDSV3(ConvFCSemanticBBoxHeadV3):

    def __init__(self, num_fcs=2, fc_out_channels=1024, *args, **kwargs):
        assert num_fcs >= 1
        super(SharedFCSemanticBBoxHeadDSV3, self).__init__(
            num_shared_convs=0,
