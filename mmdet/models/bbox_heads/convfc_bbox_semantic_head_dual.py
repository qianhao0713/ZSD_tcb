...
class ConvFCSemanticBBoxHeadDual(BBoxSemanticHeadDual):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(ConvFCSemanticBBoxHeadDual, self).__init__(*args, **kwargs)
    ...
    def init_weights(self):
        super(ConvFCSemanticBBoxHeadDual, self).init_weights()
        ...
        for i in range(self.num_path):
            if self.with_semantic:
                ...
                if self.voc is not None:
                    ...
                    semantic_score = torch.mm(semantic_score, self.vec_list[i])
                else:
                    semantic_score = self.kernel_semantic_list[i](self.vec_list[i])
...
@HEADS.register_module
class SharedFCSemanticBBoxHeadDual(ConvFCSemanticBBoxHeadDual):

    def __init__(self, num_fcs=2, fc_out_channels=1024, *args, **kwargs):
        assert num_fcs >= 1
        super(SharedFCSemanticBBoxHeadDual, self).__init__(
            num_shared_convs=0,
