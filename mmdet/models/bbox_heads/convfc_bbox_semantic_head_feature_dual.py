...
class ConvFCSemanticBBoxHeadFeatureDual(BBoxSemanticHeadFeature):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(ConvFCSemanticBBoxHeadFeatureDual, self).__init__(*args, **kwargs)
        ...
        for i in range(self.num_path):
            if self.with_semantic:
                ...
                if self.voc is not None:
                    kernel_semantic = nn.Linear(self.voc.shape[1], self.vec.shape[0])  # n*300
                    if self.with_decoder:
                        d_kernel_semantic = nn.Linear(self.vec.shape[0], self.voc.shape[1])  # n*300
                else:
                    kernel_semantic = nn.Linear(self.vec.shape[1], self.vec.shape[1])
                    if self.with_decoder:
                        d_kernel_semantic = nn.Linear(self.vec.shape[1], self.vec.shape[1])  # n*300
    ...
    def init_weights(self):
        super(ConvFCSemanticBBoxHeadFeatureDual, self).init_weights()
    ...
    def forward(self, x, res_feats=None, context_feats=None, return_feats=False, resturn_center_feats=False, bg_vector=None):
       ...
       # different branch for different vec 
            ...
            if self.with_semantic:
                ...
                if self.voc is not None:
                    ...
                    if self.semantic_norm:
                        ...
                        projected_feature = self.kernel_semantic_list[i](semantic_score)
                    if self.with_decoder:
                        d_semantic_score = self.d_kernel_semantic_list[i](projected_feature)
                        d_semantic_feature = torch.mm(d_semantic_score, self.voc.t())
                        d_semantic_feature = self.d_fc_semantic_list[i](d_semantic_feature)
                    if i == 0:
                        vec = self.vec
                    else:
                        vec = self.projection_layer(self.vec.t()).t()
                    if self.sync_bg:
                        with torch.no_grad():
                            vec[:, 0] = bg_vector
                        if not self.seen_class:
                            self.vec_unseen[:, 0] = bg_vector
                    semantic_score = torch.mm(projected_feature, vec)
                else:
                    semantic_score = self.kernel_semantic_list[i](self.vec)
        ...
        if self.with_reg and not self.reg_with_semantic:
            ...
            bbox_pred = None
        if self.with_decoder:
            return semantic_score, bbox_pred, x_semantic, d_semantic_feature_list, projected_feature
        else:
            return semantic_score, bbox_pred


@HEADS.register_module
class SharedFCSemanticBBoxHeadFeatureDual(ConvFCSemanticBBoxHeadFeatureDual):

    def __init__(self, num_fcs=2, fc_out_channels=1024, *args, **kwargs):
        assert num_fcs >= 1
        super(SharedFCSemanticBBoxHeadFeatureDual, self).__init__(
            num_shared_convs=0,
