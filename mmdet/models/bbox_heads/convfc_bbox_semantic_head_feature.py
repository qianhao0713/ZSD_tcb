...
class ConvFCSemanticBBoxHeadFeature(BBoxSemanticHeadFeature):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(ConvFCSemanticBBoxHeadFeature, self).__init__(*args, **kwargs)
    ...
    def init_weights(self):
        super(ConvFCSemanticBBoxHeadFeature, self).init_weights()
    ...
    def forward(self, x, res_feats=None, context_feats=None, return_feats=False, resturn_center_feats=False, bg_vector=None):
        ...
        if self.with_semantic:
            ...
            if self.voc is not None:
                ...
                if self.semantic_norm:
                    ...
                    projected_feature = self.kernel_semantic(semantic_score)
                if self.with_decoder:
                    d_semantic_score = self.d_kernel_semantic(projected_feature)
                    d_semantic_feature = torch.mm(d_semantic_score, self.voc.t())
                    d_semantic_feature = self.d_fc_semantic(d_semantic_feature)

                semantic_score_fix = torch.mm(projected_feature, self.vec)
                semantic_Score_dy = torch.mm(projected_feature, self.projection_layer(self.vec.t()).t())
        ...
        if self.with_reg and not self.reg_with_semantic:
            ...
            bbox_pred = None
        if self.with_decoder:
            return semantic_score, bbox_pred, x_semantic, d_semantic_feature, projected_feature
        else:
            return semantic_score, bbox_pred


@HEADS.register_module
class SharedFCSemanticBBoxHeadFeature(ConvFCSemanticBBoxHeadFeature):

    def __init__(self, num_fcs=2, fc_out_channels=1024, *args, **kwargs):
        assert num_fcs >= 1
        super(SharedFCSemanticBBoxHeadFeature, self).__init__(
            num_shared_convs=0,
