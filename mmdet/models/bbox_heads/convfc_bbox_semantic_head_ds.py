...
class ConvFCSemanticBBoxHeadDynamicSeen(BBoxSemanticHeadDynamicSeen):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(ConvFCSemanticBBoxHeadDynamicSeen, self).__init__(*args, **kwargs)
    ...
    def init_weights(self):
        super(ConvFCSemanticBBoxHeadDynamicSeen, self).init_weights()
    ...
    def forward(self, x, res_feats=None, context_feats=None, return_feats=False, resturn_center_feats=False, bg_vector=None):
        ...
        if self.with_semantic:
            ...
            vec_seen  = F.normalize(self.vec,2,0)
            if self.sync_bg:
                with torch.no_grad():
                    vec_seen[:, 0] = bg_vector
            ...
            if self.voc is not None:
                ...
                semantic_score = torch.mm(semantic_score, vec_seen)
            else:
                semantic_score = self.kernel_semantic(vec_seen)
...
@HEADS.register_module
class SharedFCSemanticBBoxHeadDynamicSeen(ConvFCSemanticBBoxHeadDynamicSeen):

    def __init__(self, num_fcs=2, fc_out_channels=1024, *args, **kwargs):
        assert num_fcs >= 1
        super(SharedFCSemanticBBoxHeadDynamicSeen, self).__init__(
            num_shared_convs=0,
