...
class BBoxSemanticHeadDynamicSeen(nn.Module):
    ...
    def __init__(self,
        ...
        super(BBoxSemanticHeadDynamicSeen, self).__init__()
        ...
        if self.with_semantic:
            ...
            vec_fix = torch.tensor(vec, dtype=torch.float32)
            self.vec_fix = vec_fix.cuda()
    ...
    def loss(self,
        ...
        #TODO npair loss
        loss_npair = self.loss_npair(self.vec[:,1:])  #self.vec[0] bg vec
...
#TODO
    ...
    def get_det_bboxes(self,
        ...
        vec_seen = self.vec_fix  #self.vec#
        vec_seen[:,1:]=F.normalize(vec_seen[:,1:],2,0)
        self.vec_unseen[:,1:] = F.normalize(self.vec_unseen[:,1:],2,0) 
        if self.gzsd:
            seen_scores = torch.mm(scores, vec_seen.t())
            seen_scores = torch.mm(seen_scores, vec_seen)
            seen_bboxes = delta2bbox(rois[:, 1:], bbox_pred, self.target_means,
                                     self.target_stds, img_shape)

            unseen_scores = torch.mm(scores, vec_seen.t())
        ...
        if self.seen_class:
            scores = torch.mm(scores, vec_seen.t())
            scores = torch.mm(scores, vec_seen)
    ...
    def get_det_bboxes_zsd(self,
                       ...
                       rescale=False,
                       cfg=None):
        if isinstance(seen_score, list):
            seen_score = sum(seen_score) / float(len(seen_score))
        seen_score = F.softmax(seen_score, dim=1) if seen_score is not None else None
        # scores = LSoftmaxLinear(semantic_score, dim=1) if semantic_score is not None else None
        if isinstance(unseen_score, list):
            unseen_score = sum(unseen_score) / float(len(unseen_score))
        unseen_score = F.softmax(unseen_score, dim=1) if unseen_score is not None else None

        if self.gzsd:
            seen_scores = torch.mm(seen_score, self.vec.t())
            seen_scores = torch.mm(seen_scores, self.vec)
            seen_bboxes = delta2bbox(rois[:, 1:], seen_bbox_pred, self.target_means,
                                     self.target_stds, img_shape)

            unseen_scores = torch.mm(unseen_score, self.vec_unseen.t())
            unseen_scores = torch.mm(unseen_scores, self.vec_unseen)
            unseen_bboxes = delta2bbox(rois[:, 1:], unseen_bbox_pred, self.target_means,
                                     self.target_stds, img_shape)
        ...
        # TODO ZSD  open these lines when unseen inference
        if not self.seen_class:
            unseen_score = torch.mm(unseen_score, self.vec_unseen.t())
            unseen_scores = torch.mm(unseen_score, self.vec_unseen)

            
        if unseen_bbox_pred is not None:
            bboxes = delta2bbox(rois[:, 1:], unseen_bbox_pred, self.target_means,
                                self.target_stds, img_shape)
        else:
            bboxes = rois[:, 1:].clone()
            if img_shape is not None:
        ...
        if cfg is None:
            return bboxes, unseen_scores
        else:
            det_bboxes, det_labels = multiclass_nms(bboxes, unseen_scores,
                                                    cfg.score_thr, cfg.nms,
                                                    cfg.max_per_img)
